...
### 6.2 设备信息查询处理
```c
uint16_t device_info_read_callback(uint8_t *buffer, uint16_t buffer_size) {
    if (buffer_size < 6) return 0;
    
    buffer[0] = 0xB0;                    // header
    buffer[1] = 0x00;                    // cmd
    buffer[2] = 0x01;                    // motor_count (1 motor)
    buffer[3] = FIRMWARE_VERSION_LOW;    // fw_version_low
    buffer[4] = FIRMWARE_VERSION_HIGH;   // fw_version_high
    buffer[5] = get_battery_level();     // battery_level (0-100)
    
    return 6;  // Return number of bytes
}
```
