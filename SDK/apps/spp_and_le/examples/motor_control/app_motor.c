...
#define LOG_TAG_CONST       MOTOR_CONTROL
...
static u8 is_app_motor_active = 0;
 ...
 */
/*************************************************************************************************/
void motor_power_event_to_user(u8 event)
{
    struct sys_event e;
...
static void motor_set_soft_poweroff(void)
{
    log_info("set_soft_poweroff\n");
    is_app_motor_active = 1;
...
static void motor_timer_handle_test(void)
{
    log_info("not_bt");
...
static const ble_init_cfg_t motor_ble_config = {
...
#if TCFG_USER_BLE_ENABLE
        btstack_ble_start_before_init(&motor_ble_config, 0);
#endif

        btstack_init();

#if TCFG_USER_BLE_ENABLE
        /* Initialize motor BLE control */
        motor_ble_module_enable(1);
#endif

#else
//no bt,to for test
        sys_timer_add(NULL, motor_timer_handle_test, 1000);
#endif
    }
    /* 按键消息使能 */
 ...
 */
/*************************************************************************************************/
static int motor_state_machine(struct application *app, enum app_state state, struct intent *it)
{
    switch (state) {
    case APP_STA_CREATE:
        break;
 ...
 */
/*************************************************************************************************/
static int motor_bt_hci_event_handler(struct bt_event *bt)
{
 ...
 */
/*************************************************************************************************/
static int motor_bt_connction_status_event_handler(struct bt_event *bt)
{
 ...
 */
/*************************************************************************************************/
static void motor_key_event_handler(struct sys_event *event)
{
    /* u16 cpi = 0; */
    u8 event_type = 0;
    u8 key_value = 0;
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
        ...
        if (event_type == KEY_EVENT_TRIPLE_CLICK
            ...
            //for test
            motor_power_event_to_user(POWER_EVENT_POWER_SOFTOFF);
            return;
        }
 ...
 */
/*************************************************************************************************/
static int motor_event_handler(struct application *app, struct sys_event *event)
{
    switch (event->type) {
    case SYS_KEY_EVENT:
        motor_key_event_handler(event);
        return 0;

    case SYS_BT_EVENT:
#if (TCFG_USER_EDR_ENABLE || TCFG_USER_BLE_ENABLE)
        if ((u32)event->arg == SYS_BT_EVENT_TYPE_CON_STATUS) {
            motor_bt_connction_status_event_handler(&event->u.bt);
        } else if ((u32)event->arg == SYS_BT_EVENT_TYPE_HCI_STATUS) {
            motor_bt_hci_event_handler(&event->u.bt);
        }
#endif
        return 0;
    ...
    case SYS_DEVICE_EVENT:
        if ((u32)event->arg == DEVICE_EVENT_FROM_POWER) {
            return app_power_event_handler(&event->u.dev, motor_set_soft_poweroff);
        }
...
static const struct application_operation app_motor_ops = {
    .state_machine  = motor_state_machine,
    .event_handler 	= motor_event_handler,
};

/*
 * Register Motor Control Application
 */
REGISTER_APPLICATION(app_motor) = {
    .name 	= "motor_control",
    .action	= ACTION_MOTOR_MAIN,
    .ops 	= &app_motor_ops,
 ...
 */
/*************************************************************************************************/
static u8 motor_state_idle_query(void)
{
    return !is_app_motor_active;
}

REGISTER_LP_TARGET(motor_state_lp_target) = {
    .name = "motor_state_deal",
    .is_idle = motor_state_idle_query,
};
